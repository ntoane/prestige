...
<div class="row">
    ...
    <div class="col-md-12 text-right noprint">
        <button onclick="javascript:window.print()" type="button" class="btn btn-success btn-sm"><i
                class="fa fa-print"></i> Print Payslip</button>
    </div>
</div>
<br>
<div class="tile mr-3 ml-3 watermark">
    <div class="row">

        <div class="text-left col-sm-12">
            <?php
$emp = $this->employees->get_emp_details($emp_id);
?>
            <table class="table">
                <thead>
                    <tr>
...
$sunday_pay = $this->payroll->get_sum_of_sundaypays($emp_id, $payment_month);
?></h5>
                        </th>
                        <th>
                            <h6>Employee Name: <span><?=$emp->fullname;?></span></h6>
                            <h6>Employement Number:
                                <span><?=$this->employees->get_emp_number($emp_id);?></span>
                            </h6>
...
if ($payslip->base_salary != 0) {
    ?>
                    ...
                    <?php }
$scheduled_allowances = $this->payroll->get_row("SELECT * FROM tbl_loans l INNER JOIN tbl_track_loan tl ON l.loan_id = tl.loan_id  WHERE l.emp_id = " . $emp_id . " AND l.loan_type ='allowance' AND tl.pay_month='" . $payment_month . "'");
$onceoff_allowances = $this->payroll->get_payslip_allowances($emp_id, $payment_month);
...
if ($emp->premium > 0) {
    ?>
                    <tr>
                        <td>PREMIUM</td>
                        <td><?=_format_money($this, $emp->premium, true)?></td>
                    </tr>

                    <?php }
if ($emp->staff_party > 0) {
    ?>
                    <tr>
                        <td>STAFF PARTY</td>
                        <td><?=_format_money($this, $emp->staff_party, true)?></td>
                    </tr>
                    <?php }?>
                    <?php
$scheduled_deductions = $this->payroll->get_row("SELECT * FROM tbl_loans l INNER JOIN tbl_track_loan tl ON l.loan_id = tl.loan_id WHERE l.emp_id = " . $emp_id . " AND l.loan_type ='deduction' AND tl.pay_month='" . $payment_month . "'");
$onceoff_deductions = $this->payroll->get_payslip_deductions($emp_id, $payment_month);
...
if ($payslip->long_term > 0) {
    ?>
                    <tr>
                        <td>LONG TERM</td>
                        <td><?=_format_money($this, $payslip->long_term, true);?></td>
                    </tr>
                    <?php
}
if ($payslip->short_term > 0) {
    ?>
                    <tr>
                        <td>SHORT TERM</td>
                        <td><?=_format_money($this, $payslip->short_term, true);?></td>
                    </tr>
                    <?php
}
...
?>
                    ...
                    <tr>
                        <?php $total_deductions = $payslip->loan_deduction + $payslip->fine_deduction + $payslip->tax + $payslip->long_term + $payslip->short_term + $emp->premium + $emp->staff_party;?>
        ...
        <div class="col-sm-4">
            <table class="table">
                ...
                <tbody>
                    <tr>
                        <?php
$emp_bank = $this->employees->get_emp_details($emp_id);
if (!empty($emp_bank)) {
    ?>
                        <td>
                            <?php
        ...
        <div class="table-responsive col-sm-5">
            <table class="table table-bordered">
                ...
                <tbody>
                    <tr class="text-center">
                        <!-- <td>
                            <?=_format_money($this, $this->payroll->get_sum_of_long_term($emp_id, $payment_month), true);?>
                        </td> -->
                        <td>
                            <!-- <?=_format_money($this, $this->payroll->get_sum_of_short_term($emp_id, $payment_month), true);?> -->
                            <?=_format_money($this, $this->payroll->get_short_term_balance($emp_id, $payment_month), true);?>
                        </td>
                    </tr>
                </tbody>
            </table>
...
$income_taxable = $total_income - $payslip->long_term;
